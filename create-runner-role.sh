...
DEPLOY_OUTPUT=$(aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    OidcProviderArn="arn:aws:iam::${AWS_ACCOUNT_ID}:oidc-provider/api.bitbucket.org/2.0/workspaces/${WORKSPACE_NAME}/pipelines-config/identity/oidc" 2>&1)
DEPLOY_EXIT=$?
set -e
